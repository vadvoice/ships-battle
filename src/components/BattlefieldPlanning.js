...
import { COLOR_SCHEMA, GAME_STAGES, INITIAL_BATTLEFIELD_SETUP, SHIP_DETAILS } from '@/libs/config';
import {
  buildTableContent,
  getGenericFleet,
...
export default function BattlefieldPlanning({
  ...
  const resetFleet = () => {
    ...
    onChange({ ...gameState, [autoGenerated ? 'enemy' : 'player']: INITIAL_BATTLEFIELD_SETUP });
    // onChange({ ...gameState, [`player${position}`]: initialBattlefieldSetup });
  };
  ...
  const onReady = () => {
    ...
    onChange({ ...gameState, [autoGenerated ? 'enemy' : 'player']: battlefield });
